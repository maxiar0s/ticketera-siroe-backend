...
const emailTicketConfig = {
  ...
  pollIntervalMs: parseInteger(process.env.TICKET_INBOUND_POLL_INTERVAL_MS, 60000),
  maxEmailsPerRun: parseInteger(process.env.TICKET_INBOUND_MAX_EMAILS_PER_RUN, 10),
  defaultTechnicians: parseList(process.env.TICKET_INBOUND_DEFAULT_TECHNICIANS, ["Mesa de ayuda"]),
  fallbackCasaMatrizId: process.env.TICKET_INBOUND_FALLBACK_CLIENT_ID ?? null,
  defaultCreatorAccountId: parseInteger(process.env.TICKET_INBOUND_FALLBACK_CREATOR_ID, null),
  useSenderAccountAsCreator: toBoolean(
    process.env.TICKET_INBOUND_USE_SENDER_ACCOUNT_AS_CREATOR,
    true
  ),
  markSeenOnError: toBoolean(process.env.TICKET_INBOUND_MARK_SEEN_ON_ERROR, false),
  allowedSenderEmails: parseList(process.env.TICKET_INBOUND_ALLOWED_SENDER_EMAILS, []),
  ...
  smtpSecure: toBoolean(process.env.TICKET_OUTBOUND_SMTP_SECURE ?? "true", true),
  smtpUser: process.env.TICKET_OUTBOUND_SMTP_USER ?? "",
  smtpPassword: process.env.TICKET_OUTBOUND_SMTP_PASSWORD ?? "",
  outboundFromAddress:
    process.env.TICKET_OUTBOUND_FROM_ADDRESS ?? process.env.TICKET_INBOUND_SMTP_USER ?? "",
  outboundFromName: process.env.TICKET_OUTBOUND_FROM_NAME ?? "Mesa de Ayuda",
  outboundSubjectPrefix: process.env.TICKET_OUTBOUND_SUBJECT_PREFIX ?? "[Ticket creado]",
